...
TEST_RUNNER.run({
  ...
  cases: [
    ...
    new (class implements TestCase {
      public name = "TopDown_ClearedAndAddedMore_ApplySettings";
      private cut: DanmakuOverlay;
      public async execute() {
        // Prepare
        await setTabletView();
        ...
        // Verify
        await asyncAssertScreenshot(
          path.join(__dirname, "/danmaku_overlay_cleared_and_added_more.png"),
          path.join(
            __dirname,
            "/golden/danmaku_overlay_cleared_and_added_more.png",
          ),
          path.join(
            __dirname,
            "/danmaku_overlay_cleared_and_added_more_diff.png",
          ),
        );
