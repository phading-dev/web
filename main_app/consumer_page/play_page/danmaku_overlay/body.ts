...
import { DENSITY_SCALE } from "../common/defaults";
...
export class DanmakuOverlay {
  public static create(settings: CommentOverlaySettings): DanmakuOverlay {
  ...
  private tryStartPlaying(
    ...
    let entry = this.createDanmakuEntry(this.settings, comment.comment);
    ...
    let marginAround =
      Math.floor(
        entryHeight / (this.settings.danmakuSettings.density * DENSITY_SCALE),
      ) - entryHeight;
    let occupyScore = 0;
