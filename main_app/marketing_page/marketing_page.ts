...
import {
  MarketingPage as MarketingPageState,
  Tab,
} from "@phading/web_interface/main/marketing/page";
import { E } from "@selfage/element/factory";
import { Ref } from "@selfage/ref";

export interface MarketingPage {
  on(event: "signIn", listener: (accountType: AccountType) => void): this;
  on(event: "pricing", listener: () => void): this;
}
...
export class MarketingPage extends EventEmitter {
  ...
  private state: MarketingPageState;
  ...
  public constructor(private getDateNow: () => Date) {
    ...
    this.body = E.div(
      ...
      E.divRef(
        this.consumerTabContent,
        {
          ...
          style: `flex-flow: column nowrap; padding: 3rem; max-width: 80rem;`,
        },
        E.div(
          {
        ...
        E.divRef(
          this.consumerSignInButton,
          {
            ...
            style: `${FILLED_BUTTON_STYLE} align-self: center; background-color: ${SCHEME.primary1};`,
          },
      ...
      E.divRef(
        this.publisherTabContent,
        {
          ...
          style: `flex-flow: column nowrap; padding: 2rem;`,
        },
        E.div(
          {
    ...
    this.consumerSignInButton.val.addEventListener("click", () =>
      this.emit("signIn", AccountType.CONSUMER),
    );
    this.publisherSignInButton.val.addEventListener("click", () =>
      this.emit("signIn", AccountType.PUBLISHER),
    );
  ...
  public updateState(state: MarketingPageState): void {
    if (!state) {
      state = {};
    }
    if (!state.tab) {
      state.tab = Tab.CONSUMER;
    }
    this.state = state;
    this.tabNavigator.goTo(this.state.tab);
  }
