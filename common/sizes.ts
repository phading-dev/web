export let AVATAR_S = 4; // rem
export let AVATAR_M = 10; // rem

export let FONT_S = 1.4; // rem
export let FONT_M = 1.6; // rem
export let FONT_L = 1.8; // rem
export let FONT_XL = 2.4; // rem

export let FONT_WEIGHT_600 = 600;

export let LINE_HEIGHT_M = 1.8; // rem
export let LINE_HEIGHT_L = 2; // rem
export let LINE_HEIGHT_XL = 2.6; // rem
export let LINE_HEIGHT_XXL = 3; // rem. The same as ICON_BUTTON_M

export let ICON_XS = 1; // rem
export let ICON_S = 1.2; // rem
export let ICON_M = 1.4; // rem
export let ICON_L = 1.6; // rem
export let ICON_XL = 2.4; // rem
export let ICON_XXL = 3.2; // rem

export let ICON_BUTTON_M = 3; // rem
export let ICON_BUTTON_L = 4; // rem
export let ICON_BUTTON_XL = 5; // rem

export let DATE_INPUT_WIDTH = 15; // rem
export let DATETIME_INPUT_WIDTH = 20; // rem
