...
export class PlayPage extends EventEmitter {
  public static create(show: Show): PlayPage {
    return new PlayPage(
      window,
  ...
  public constructor(
    ...
    private createCommentsCard: (showId: string) => CommentsCard,
    private createInfoCard: (show: Show) => InfoCard,
    private createSettingsCard: (
      playerSettings: PlayerSettings,
    ) => SettingsCard,
    private createCommentsPool: (showId: string) => CommentsPool,
    private show: Show,
  ) {
    super();
    this.body = E.div({
    ...
    this.commentsPool = this.createCommentsPool(this.show.showId);
    this.load();
  }

  private async load(): Promise<void> {
    let reponse = await getPlayerSettings(this.webServiceClient, {});
    this.playerSettings = PlayPage.normalizePlayerSettings(
      reponse.playerSettings,
    );
    this.body.append(
      assign(this.player, this.createPlayer(this.playerSettings, this.show))
        .body,
      E.divRef(
        this.cardContainer,
        {
        ...
        assign(
          this.commentsCard,
          this.createCommentsCard(this.show.showId).hide(),
        ).body,
        assign(this.infoCard, this.createInfoCard(this.show).hide()).body,
        assign(
          this.settingsCard,
    ...
    this.closeCard();
    this.commentsCard.val.setCallbackToGetTimestamp(() =>
      this.player.val.getCurrentVideoTimestamp(),
    );
    ...
    this.infoCard.val.on("focusUser", (accountId) =>
      this.emit("focusUser", accountId),
    );
  ...
  private startPlaying(): void {
    this.commentsPool.startFrom(this.player.val.getCurrentVideoTimestamp());
    this.addCommentsPeriodically();
  }

  private addCommentsPeriodically = (): void => {
    let timestamp = this.player.val.getCurrentVideoTimestamp();
    let comments = this.commentsPool.read(timestamp);
    this.commentsCard.val.addComments(comments);
    this.player.val.addDanmaku(comments);
    this.addCommentsFrameId = this.window.requestAnimationFrame(
      this.addCommentsPeriodically,
    );
  };

  private stopPlaying(): void {
    this.window.cancelAnimationFrame(this.addCommentsFrameId);
  }
